...
def run_phase(phase_num: int, phase_name: str, command: list, cwd: Path) -> bool:
    ...
    try:
        ...
        print(f"\n❌ Phase {phase_num} failed: {phase_name}")
        print(f"Error: {e}")
        return False
...
def main():
    ...
    print("="*80)
    print("🚀 AUTONOMOUS PRODUCT DEMO GENERATOR (PRO EDITION)")
    print("="*80)
    ...
    if not run_phase(
        ...
        print("❌ Recording failed. Pipeline stopped.")
        sys.exit(1)
    ...
    if not run_phase(
        ...
        print("\n❌ Pipeline failed at Phase 3 (Hook Scene)")
        sys.exit(1)
    ...
    if not run_phase(
        ...
        print("\n❌ Pipeline failed at Phase 3 (Tech Stack Scene)")
        sys.exit(1)
